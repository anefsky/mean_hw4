...
app.controller("NavController", function( $scope, $http ) {
  ...
  $http.get('https://api.github.com/users/anefsky').then(onUserComplete)

  function onUserComplete(response) {
   $scope.user = response.data
  } 

  $http.get('https://api.punkapi.com/v2/beers/').then(onBeerListComplete)

  function onBeerListComplete(response) {
    $scope.beerItems = response.data
    console.log($scope.beerItems)
  } 
